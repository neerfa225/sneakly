...
const updateCart = async (productId, newQty, skuId = null) => {
  ...
  const totalPrice = cart.reduce(
    (sum, item) =>
      sum +
      ((item.product?.sku?.price || item.price) * (item.qty || item.quantity || 1)),
    0
  );
      ...
      <div className="w-[90%] mx-auto mt-[140px] flex flex-wrap justify-center gap-[60px] sm:flex sm:justify-between">
        <div className="w-[90%] sm:w-[50%]">
          {cart.map((item) => (
            ...
            >
           <Link to={`/product/${item.product?.id}/${item.sku}`}>  <div className="flex items-center gap-4">
            {/* <Link to={`/product/${item.product?.id || item.id}/${item.sku?.id || item.sku}`}> */}
...
>
  +
                ...
                <button
                  onClick={() =>
                    removeItem(item.product?.id || item.productId, item.sku || item.skuId)
                  }
